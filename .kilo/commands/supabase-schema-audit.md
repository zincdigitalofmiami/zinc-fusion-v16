...
Read the full skill file at `.github/skills/supabase-schema-audit/SKILL.md` and follow it exactly — including all loop structure, per-schema loop, migration loop for failures, commit after each clean schema, final re-audit, and Hard Rules. Do not skip or abbreviate any loop.
